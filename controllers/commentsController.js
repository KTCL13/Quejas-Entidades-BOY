...
exports.getCommentsByComplaintIdController = async (req, res, next) => {
  try {
    ...
    logger.info(
      `Comentarios obtenidos exitosamente - Queja: ${complaintId}, Total: ${comments.length}`
    );
...
exports.createCommentByComplaintIdController = async (req, res, next) => {
  try {
    ...
    logger.info(
      `Comentario creado exitosamente - Queja: ${complaintId}, Comentario ID: ${newComment.id}`
    );
