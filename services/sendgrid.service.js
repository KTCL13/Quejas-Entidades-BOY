...
class SendGridService extends IMailService {
    constructor() {
        ...
        console.log('✅ Servicio de correo con SendGrid API inicializado.');
    }
    ...
    async sendMail({ to, subject, text, html }) {
        ...
        try {
            ...
            console.log('Correo enviado exitosamente con la API de SendGrid.');
            return response[0];
        } catch (error) {
            console.error('❌ Error al enviar correo con la API de SendGrid:', error.message);
            if (error.response) {
